    ...
    open: function (cluster, bucketName, password, options) {
        ...
        COUCHBASE_BUCKET_PROPERTIES.forEach(function(couchbaseBucketProperty) {
            if (options[couchbaseBucketProperty] !== undefined) {
                _bucket[couchbaseBucketProperty]  = options[couchbaseBucketProperty];
            }
        });

        function bucket(bucket, queuedCallbacks, callback) {
            if (bucket.connected) {
            ...
            if(bucket.listeners('connect').length === 0) {
                bucket.once('connect', function () {
                    var queuedCallback;
                    while(queuedCallback = queuedCallbacks.shift()) {
                        queuedCallback(null, bucket);
                    }
                });
            }

            if(bucket.listeners('error').length === 0) {
